...
struct string_view path_sv_basename(const struct string_view path) {
    ...
    const int64_t last_slash_index = sv_find_char_rev(path, '/');
    if (last_slash_index == SV_NPOS) {
        return path;
    }
    ...
    const uint32_t index = (uint32_t)(last_slash_index + 1);
...
struct string_view path_sv_extension(const struct string_view path) {
    const struct string_view basename = path_sv_basename(path);
    const int64_t last_dot_index = sv_find_char_rev(basename, '.');

    if (last_dot_index == SV_NPOS) {
        return SV_EMPTY();
    }

    const uint32_t index = (uint32_t)(last_dot_index + 1);
...
struct string_view
path_cstr_get_first_component(const char *path, const bool skip_root) {
...
__debug_optimize(3)
struct string_view
path_sv_get_first_component(struct string_view path, const bool skip_root) {
    if (__builtin_expect(sv_is_empty(path), 1)) {
        return SV_EMPTY();
    }
...
{
    if (__builtin_expect(*path == '\0' || sv_is_empty(prev_component), 0)) {
        return SV_EMPTY();
    }
...
path_sv_get_next_component(const struct string_view path,
                           const struct string_view prev_component)
{
    if (__builtin_expect(sv_is_empty(path) || sv_is_empty(prev_component), 0)) {
        return SV_EMPTY();
    }
    ...
    const int64_t next_slash_index = sv_find_char(remaining_path, '/');
    if (next_slash_index == SV_NPOS) {
        return remaining_path;
    }

    return sv_substring_upto(remaining_path, (uint32_t)next_slash_index);
}
