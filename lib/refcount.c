...
__debug_optimize(3) void refcount_init(struct refcount *const ref) {
    ref->count = 1;
}
...
__debug_optimize(3) void ref_up(struct refcount *const ref) {
    refcount_increment(ref, /*amount=*/1);
}

__debug_optimize(3) bool ref_down(struct refcount *const ref) {
    return refcount_decrement(ref, /*amount=*/1);
}

__debug_optimize(3) uint32_t ref_get(struct refcount *const ref) {
