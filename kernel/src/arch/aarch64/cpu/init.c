/*
 * kernel/src/arch/aarch64/cpu/init.c
 * © suhas pai
 */
...
void cpu_init_for_smp(struct cpu_info *const cpu) {
    ...
    void *const stack_ptr =
        page_to_virt(cpu->irq_stack)
      + ((PAGE_SIZE << KERNEL_IRQ_STACK_ORDER) - 1);

    asm volatile ("msr SP_EL1, %0" :: "r"(stack_ptr));
#endif
}
...
#if 0
    void *const stack_ptr =
        page_to_virt(this_cpu()->irq_stack)
      + ((PAGE_SIZE << KERNEL_IRQ_STACK_ORDER) - 1);

    // asm volatile ("msr spsel, #1; msr SP_EL1, %0" :: "r"(stack_ptr));
#endif
}
