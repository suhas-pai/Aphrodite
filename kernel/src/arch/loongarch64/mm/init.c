/*
 * kernel/src/arch/loongarch64/mm/init.c
 * © suhas pai
 */
             ...
             const uint64_t large_pte_flags)
{
    const struct pgmap_options options = {
        ...
        .large_pte_flags = large_pte_flags,
...
static void setup_pagestructs_table() {
    ...
    const uint64_t large_pte_flags = __PTE_WRITE | __PTE_GLOBAL_LARGE;

    alloc_region(PAGE_OFFSET, map_size, pte_flags, large_pte_flags);
                        ...
                        uint64_t pte_flags)
{
    const struct pgmap_options options = {
        ...
        .large_pte_flags = __PTE_GLOBAL_LARGE | options.leaf_pte_flags,
...
static void setup_kernel_pagemap(uint64_t *const kernel_memmap_size_out) {
    const uint64_t higher_root_phys = early_alloc_page();
    if (higher_root_phys == INVALID_PHYS) {
        panic("mm: failed to allocate higher root page for the kernel-pagemap");
    }

    const uint64_t lower_root_phys = early_alloc_page();
    if (lower_root_phys == INVALID_PHYS) {
        panic("mm: failed to allocate lower root page for the kernel-pagemap");
    }

    kernel_process.pagemap.lower_root = phys_to_virt(higher_root_phys);
    kernel_process.pagemap.higher_root = phys_to_virt(lower_root_phys);
