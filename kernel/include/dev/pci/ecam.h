/*
 * kernel/include/dev/pci/drivers/ecam.h
 * © suhas pai
 */

#pragma once

#include <lib/adt/range.h>
#include "dev/pci/domain.h"

struct pci_domain_ecam {
    struct pci_domain domain;

    struct range bus_range;
    struct mmio_region *mmio;
};

struct pci_domain_ecam *
pci_add_ecam_domain(struct range bus_range,
                    struct bus *const bus,
                    uint64_t base_addr,
                    uint16_t segment);

void pci_remove_ecam_domain(struct pci_domain_ecam *ecam_domain);

uint64_t
pci_ecam_domain_loc_get_base(const struct pci_domain_ecam *domain,
                             const struct pci_location *loc);

uint8_t
pci_ecam_read_8(const struct pci_domain_ecam *domain,
                const struct pci_location *loc,
                uint16_t offset);

uint16_t
pci_ecam_read_16(const struct pci_domain_ecam *domain,
                 const struct pci_location *loc,
                 uint16_t offset);

uint32_t
pci_ecam_read_32(const struct pci_domain_ecam *domain,
                 const struct pci_location *loc,
                 uint16_t offset);

uint64_t
pci_ecam_read_64(const struct pci_domain_ecam *domain,
                 const struct pci_location *loc,
                 uint16_t offset);

void
pci_ecam_write_8(const struct pci_domain_ecam *domain,
                 const struct pci_location *loc,
                 uint16_t offset,
                 uint8_t value);

void
pci_ecam_write_16(const struct pci_domain_ecam *domain,
                  const struct pci_location *loc,
                  uint16_t offset,
                  uint16_t value);

void
pci_ecam_write_32(const struct pci_domain_ecam *domain,
                  const struct pci_location *loc,
                  uint16_t offset,
                  uint32_t value);

void
pci_ecam_write_64(const struct pci_domain_ecam *domain,
                  const struct pci_location *loc,
                  uint16_t offset,
                  uint64_t value);
